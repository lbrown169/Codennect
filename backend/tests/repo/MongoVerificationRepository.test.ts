...
test("Deleting a static verification code", async () => {
    ...
    expect(result).toBe(true);
    result = await driver.verificationRepository.DeleteVerification(
        "test@example.com"
    );
    expect(result).toBe(true);
});
...
test("Verifying a static verification code", async () => {
    ...
    expect(result).toBe(true);
    result = await driver.verificationRepository.ValidateVerification(
        "test@example.com",
        "abc12345"
    );
    expect(result).toBe(true);
...
test("Fail verifying a static verification code (doesn't exist)", async () => {
    let result = await driver.verificationRepository.ValidateVerification(
        "test@example.com",
        "def67890"
    );
    expect(result).toBe(false);
...
// This exists in the development database already
test("Fail verifying a static verification code (expired)", async () => {
    let result = await driver.verificationRepository.ValidateVerification(
        "veryoldtest@example.com",
        "abc12345"
    );
    expect(result).toBe(false);
});
