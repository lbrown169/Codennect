...
test("Tests getting a static user by Email and Password", async () => {
    ...
    let result = await driver.userRepository.GetByEmailAndPassword("john.doe@example.com", "SuperSecret123!");
    expect(result).toBeDefined();
})
...
test("Tests failing to register a static user", async () => {
    ...
    expect(async () => {
        await driver.userRepository.Register(new UserRegistration("Jane Smith", "jane.doe@example.com", "AnotherPassword!"))
