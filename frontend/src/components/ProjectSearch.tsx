...
function ProjectSearch()
{
    ...
    const doSearch = async () =>
    {
        try
        {
            ...
            const response = await fetch(buildPath('/api/get-all-projects' + query),
            {
                method: 'GET',
        ...
        <div className='flex m-20 gap-10 items-start overflow-hidden'>
            <div className="flex flex-col w-1/3 bg-[#EFF6E0] rounded-2xl text-black">
                ...
                <div className="flex flex-col items-stretch p-5 gap-2">
                    <p className='italic'>Just a search away from your next big idea...</p>
                    <form action={doSearch} className="flex items-center gap-2">
                        <input className="grow px-2 py-1" type="text" id="searchBar" placeholder="Search Projects" value={searchInfo} onChange={handleSetSearchInfo} />
                        <button className="bg-[#598392] text-white px-2 py-2 rounded-[10px] hover:bg-[#90b0bb] hover:cursor-pointer" type="submit">
                            <FaSearch />
                        </button>
                    </form>
                </div>
            ...
            <div className='flex flex-col w-2/3 max-h-full bg-[#EFF6E0] rounded-2xl overflow-hidden'>
                ...
                <div className="overflow-y-scroll">
                    {projects.map((project) => (
                        <div key={project._id} className="m-4 border-white border-4 rounded-xl text-black">
                            <div className="flex bg-white items-center justify-between py-2 px-4 gap-2">
                                <Link to={`/projects/${project._id}`}>
                                    <h3 className="font-bold text-xl">{getCapacityString(project)} {project.name}</h3>
                                </Link>
                                <Link to={`/projects/${project._id}`}>
                            ...
                            <div className="flex flex-col py-4 px-6 gap-5 justify-between">
                                <div className="flex flex-col flex-[1_0_0] justify-center">
                                    ...
                                    <h2 className="font-medium text-wrap">{project.description}</h2>
                                </div>
                                <div className="flex grow gap-10 items-start">
                                    <div className="flex flex-col flex-[1_0_0] justify-center">
                                        <p className="text-gray-500 uppercase text-xs">Open Roles</p>
                                        <h2 className="font-medium text-wrap">{getOpenRoles(project)}</h2>
                                    </div>
                                    <div className="flex flex-col flex-[1_0_0] justify-center">
                                        <p className="text-gray-500 uppercase text-xs">Skills</p>
                                        <h2 className="font-medium text-wrap">{project.required_skills.join(", ") || "None"}</h2>
                                    </div>
                                </div>
                            </div>
                        </div>
