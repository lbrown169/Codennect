...
export function ProjectSidebar({ owner, members, project, setRefresh }: {owner: User | undefined, members: ProjectUsers, project: Project, setRefresh: React.Dispatch<React.SetStateAction<string>>}) {
                ...
                {isOwner() ? (
                    <div className="flex flex-col bg-[#EFF6E0] rounded-2xl text-black">
                        ...
                        <div className="flex flex-col justify-between py-3 px-5">
                            ...
                            <button className="bg-[#598392] text-white font-bold px-4 py-1 rounded-[10px] my-2 hover:bg-[#90b0bb] hover:cursor-pointer" onClick={() => setMode(Mode.REQUESTS)}>
                                    ...
                                    {invites.length === 0 && applications.length === 0 ? (
                                        <>
                                            {Object.keys(members).length > 0 ? (
                                                <div className="flex flex-col grow gap-2">
                                                    ...
                                                    <div className="flex gap-2">
                                                        {Object.keys(project.roles).map((role) => (
                                                            <label key={role} className="flex justify-between gap-1 bg-[#5c8593] px-4 py-1 text-white font-bold rounded-3xl">
                                                                <input type="checkbox" onChange={() => handleChangeRole(role)}/>
                                                                {role}
                                                            </label>
                                                        ))}
                                                    </div>
                                                ...
                                                <p>Applications are not open for this project because the owner has not created any roles.</p>
                                            )}
                                        </>
                                    ) : (invites.length === 0 ? (
                                        <>
                                            <p>Looks like you have an active application for this project. We'll send you an email when the project owner approves or denies your request!</p>
                                            <p>Roles: {applications[0].roles.join(", ")}</p>
                                        </>
                                    ) : (applications.length === 0 ? (
                                        <>
                                            <p>The owner of this project has invited you to join!</p>
                                            <p>Roles: {invites[0].roles.join(", ")}</p>
                                            <div className="flex gap-2 items-stretch">
                                                <button className="bg-[#598392] text-white font-bold px-4 py-1 rounded-[10px] hover:bg-[#90b0bb] hover:cursor-pointer" onClick={() => approve(uid, true)}>
                                                    Approve
                                                </button>
                                                <button className="bg-[#598392] text-white font-bold px-4 py-1 rounded-[10px] hover:bg-[#90b0bb] hover:cursor-pointer" onClick={() => deny(uid, true)}>
                                                    Deny
                                                </button>
                                            </div>
                                        </>
                                        ...
                                        <>
                                            ...
                                            <p>You also have a current application to join the project:</p>
                                            <p>Roles: {applications[0].roles.join(", ")}</p>
                                        </>
                                    )
                                    )
            ...
            <div className="flex flex-col bg-[#EFF6E0] rounded-2xl text-black overflow-hidden max-w-120 w-1/3">
                <div className="flex justify-between bg-[#598392] text-white py-3 px-5 rounded-t-2xl">
                    <div className="flex items-center gap-2">
                        <IoMailOpen />
                        <h3 className="font-bold uppercase">Project Requests</h3>
                    </div>
                    <button className="bg-[#598392] text-white font-bold px-4 py-1 rounded-[10px] hover:bg-[#90b0bb] hover:cursor-pointer" onClick={() => setMode(Mode.SIDEBAR)}>
                        <RiCloseLargeFill />
                    </button>
                </div>
                <div className="flex-[1_0_0] flex-col justify-between overflow-y-auto">
                    ...
                    {Object.keys(invites).length > 0 ? (
                        invites.map((invite) => (
                            <div className="flex flex-col py-3 px-5 gap-2" key={`${invite.project_id}-${invite.user_id}`}>
                                <div className="flex justify-between">
                                    <div className="flex gap-2 items-center">
                                        ...
                                        <Link to={`/user/${invite.user_id}`}>
                                            <h3>{userCache[invite.user_id] ?? invite.user_id}</h3>
                                        </Link>
                                    </div>
                                    <div className="flex gap-2 items-center">
                                        <button className="bg-[#598392] text-white font-bold px-4 py-1 rounded-[10px] hover:bg-[#90b0bb] hover:cursor-pointer" onClick={() => deny(invite.user_id, true)}>
                                            Deny
                                        </button>
                                    </div>
                                </div>
                    ...
                    <div className="flex bg-white items-center py-3 px-5 gap-2">
                        <h3 className="font-bold uppercase italic">Applications</h3>
                    </div>
                    {Object.keys(applications).length > 0 ? (
                        applications.map((application) => (
                            <div className="flex flex-col py-3 px-5 gap-2" key={`${application.project_id}-${application.user_id}`}>
                                <div className="flex justify-between">
                                    <div className="flex gap-2 items-center">
                                        <div className="bg-[#5c8593] text-white rounded-4xl p-2">
                                            <GoPersonFill className="w-4 h-4"/>
                                        </div>
                                        <Link to={`/user/${application.user_id}`}>
                                            <h3>{userCache[application.user_id] ?? application.user_id}</h3>
                                        </Link>
                                    </div>
                                    <div className="flex gap-2 items-center">
                                        <button className="bg-[#598392] text-white font-bold px-4 py-1 rounded-[10px] hover:bg-[#90b0bb] hover:cursor-pointer" onClick={() => approve(application.user_id, false)}>
                                            Approve
                                        </button>
                                        <button className="bg-[#598392] text-white font-bold px-4 py-1 rounded-[10px] hover:bg-[#90b0bb] hover:cursor-pointer" onClick={() => deny(application.user_id, false)}>
                                            Deny
                                        </button>
                                    </div>
                                </div>
